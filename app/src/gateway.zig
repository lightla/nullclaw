    ...
    if (ctx.state.event_bus) |eb| {
        ...
        if (session_mod.isHelpCommand(text)) {
            ...
            if (help_text) |result| {
                ...
                const wrapped = std.fmt.allocPrint(ctx.root_allocator, "```\n{s}\n```", .{result}) catch result;
                defer if (wrapped.ptr != result.ptr) ctx.root_allocator.free(wrapped);
                var outbound_ch = channels.slack.SlackChannel.initFromConfig(ctx.req_allocator, slack_cfg.*);
                outbound_ch.sendMessage(channel_id, wrapped) catch {};
            }
            ctx.response_body = "{\"status\":\"ok\"}";
            return;
        }
