...
fn inboundMsgWorker(ctx: *InboundMsgWorkerCtx) void {
    ...
    if (session_mod.isHelpCommand(msg.content)) {
        ...
        const wrapped = std.fmt.allocPrint(allocator, "```\n{s}\n```", .{help_text}) catch help_text;
        defer if (wrapped.ptr != help_text.ptr) allocator.free(wrapped);
        const out = (if (outbound_account_id) |aid|
            bus_mod.makeOutboundWithAccount(allocator, msg.channel, aid, msg.chat_id, wrapped)
        else
            bus_mod.makeOutbound(allocator, msg.channel, msg.chat_id, wrapped)) catch return;
